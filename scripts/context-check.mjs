...
const required = [
  ...
  'AI-RELEASE-EXECUTION-PLAN.md', 'RELEASE-ROADMAP.md',
  'IMPLEMENTATION-AUDIT-AND-MICROPHASE-PLAN.md', 'phase-manifest.v2.json',
...
for (const marker of ['PROJECT_CONTEXT.md', 'STATUS.md', 'docs/workflow/README.md', 'scripts/workflow.mjs']) if (!agents.includes(marker)) fail(`AGENTS.md missing route ${marker}`);
for (const marker of ['AI-RELEASE-EXECUTION-PLAN.md', 'RELEASE-ROADMAP.md', 'docs/OzelAPP_Derin_Arastirma_2026-09-03/']) if (!context.includes(marker)) fail(`PROJECT_CONTEXT.md missing route ${marker}`);
for (const marker of ['P-12B', 'P-12C', 'P-12D', 'release açık değil']) if (!status.includes(marker)) fail(`STATUS.md missing current marker ${marker}`);
