...
assert(route.includes('booths'), 'stand kodlari donmeli')

// View: event guard + durumlar + endpoint; website metin, baglanti yok
assert(view.includes('data-testid="sponsor-view"'), 'panel test kimligi tasimali')
assert(view.includes('/sponsors'), 'sponsor endpointini cagirmali')
assert(view.includes('Önce etkinlik seçin'), 'event-gerekcesi olmali')
assert(view.includes('Sponsorlar yükleniyor'), 'yukleme durumu olmali')
assert(view.includes('sponsor yok'), 'bos durum olmali')
...
assert(types.includes("| 'sponsors'"), 'AppView sponsors tasimali')
assert(sidebar.includes("id: 'sponsors'") && sidebar.includes('Sponsorlar'), 'sidebar Sponsorlar ogesi tasimali')
assert(shell.includes('<SponsorView />'), 'shell sponsor render etmeli')

console.log('ux-sponsor-view.test: PASS (sponsor salt-okunur kilitli)')
