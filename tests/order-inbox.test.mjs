...
assert(route.includes('itemCount: row._count.items'), 'kalem sayimi donmeli')
...
// POST create kurallari degismedi
assert(route.includes('can.manageBilling'), 'POST billing gate korunmali')
assert(route.includes('orderItemSchema'), 'POST kalem semasi korunmali')

// Canli: anon sizdirmadan 401
const anon = await fetch('http://127.0.0.1:3000/api/orders?eventId=x')
...
const bogus = await fetch('http://127.0.0.1:3000/api/orders?eventId=does-not-exist', { headers: { Authorization: `Bearer ${session.data.token}` } })
assert(bogus.status === 404, `bogus event 404 olmali, got ${bogus.status}`)

console.log('order-inbox.test: PASS (siparis listesi + 401/404 kilitli)')
